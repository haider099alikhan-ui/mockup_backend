      ...
      if (challengeResponse.webauthn.type === "create") {
        ...
        if (!user2.name) {
          ...
          if (!nameToUse) {
            ...
            user2.name = `${user2.id}:${fallbackName}`;
          } else {
            user2.name = `${user2.id}:${nameToUse}`;
          }
        }
  ...
  async _getAuthenticatorAssuranceLevel(jwt) {
    ...
    if (jwt) {
      try {
        ...
        const { data: { user: user2 }, error: userError } = await this.getUser(jwt);
        if (userError) {
          return this._returnResult({ data: null, error: userError });
        }
        const verifiedFactors2 = (_b = (_a = user2 === null || user2 === void 0 ? void 0 : user2.factors) === null || _a === void 0 ? void 0 : _a.filter((factor) => factor.status === "verified")) !== null && _b !== void 0 ? _b : [];
...
async function authMiddleware(c, next) {
  ...
  try {
    ...
    const { data: { user: user2 }, error } = await supabase.auth.getUser(token);
    if (error || !user2) {
...
projects.get("/", async (c) => {
  ...
  const supabase = createSupabaseAdmin(c.env);
  const { data, error } = await supabase.from("projects").select("id, name, template_id, template_name, created_at, updated_at").eq("user_id", user2.id).order("updated_at", { ascending: false });
  if (error)
...
projects.get("/:id", async (c) => {
  ...
  const supabase = createSupabaseAdmin(c.env);
  const { data, error } = await supabase.from("projects").select("*").eq("id", projectId).eq("user_id", user2.id).single();
  if (error || !data)
...
projects.post("/", async (c) => {
  ...
  const body = await c.req.json();
  const supabase = createSupabaseAdmin(c.env);
  const { data: profile } = await supabase.from("profiles").select("plan").eq("id", user2.id).single();
  if (profile?.plan === "free") {
    const { count } = await supabase.from("projects").select("id", { count: "exact", head: true }).eq("user_id", user2.id);
    if (count >= 3) {
      return c.json({
...
projects.put("/:id", async (c) => {
  ...
  if (body.slides !== void 0)
    updates.slides = body.slides;
  if (body.template_id !== void 0)
    updates.template_id = body.template_id;
  if (body.template_name !== void 0)
    updates.template_name = body.template_name;
  updates.updated_at = (/* @__PURE__ */ new Date()).toISOString();
  const { data, error } = await supabase.from("projects").update(updates).eq("id", projectId).eq("user_id", user2.id).select().single();
  if (error)
    return c.json({ error: error.message }, 500);
  if (!data)
    return c.json({ error: "Project not found" }, 404);
...
projects.delete("/:id", async (c) => {
  ...
  const projectId = c.req.param("id");
  const supabase = createSupabaseAdmin(c.env);
  const { error } = await supabase.from("projects").delete().eq("id", projectId).eq("user_id", user2.id);
  if (error)
    return c.json({ error: error.message }, 500);
...
var exports = new Hono2();
var FREE_EXPORT_LIMIT = 5;
exports.post("/check", async (c) => {
  const authUser = c.get("user");
  const supabase = createSupabaseAdmin(c.env);
  const { data: profile } = await supabase.from("profiles").select("plan, exports_used, exports_reset_at").eq("id", authUser.id).single();
  if (!profile)
    return c.json({ canExport: true, remaining: FREE_EXPORT_LIMIT });
  if (profile.plan === "pro") {
  ...
  if (monthDiff >= 1) {
    await supabase.from("profiles").update({ exports_used: 0, exports_reset_at: now.toISOString() }).eq("id", authUser.id);
    exportsUsed = 0;
  }
...
exports.post("/track", async (c) => {
  const authUser = c.get("user");
  const supabase = createSupabaseAdmin(c.env);
  const { data: profile } = await supabase.from("profiles").select("plan, exports_used").eq("id", authUser.id).single();
  if (profile?.plan === "pro") {
    return c.json({ success: true });
  }
  const newCount = (profile?.exports_used || 0) + 1;
  await supabase.from("profiles").update({ exports_used: newCount }).eq("id", authUser.id);
  return c.json({
    success: true,
